...
- (NSString *)description {
    if (self.isDoubleMove) {
        return [NSString stringWithFormat:@"方块单步移动: (从 %ld 和 %ld 到 %ld , 新的值为: %ld)", self.source1, self.source2, self.destination, self.value];
    }
    else {
        return [NSString stringWithFormat:@"方块同步移动: (从 %ld 到 %ld , 新的值为: %ld)", self.source1, self.destination, self.value];
    }
}
